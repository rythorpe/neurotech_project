...
def lempel_ziv_complexity(sequences, normalize=False):
    """Compute Lempel-Ziv complexity for a 1-D list of binary sequences"""
    ...
    ind = 0
    inc = 1
    while ind + inc <= len(sequences[0]):
        new_sub_str_added = False
        for sequence in sequences:
            sub_str = sequence[ind:ind + inc]
            if sub_str not in sub_strings:
                sub_strings.add(sub_str)
                new_sub_str_added = True
            else:
                continue
        if new_sub_str_added:
            ind += inc
            inc = 1
        else:
            inc += 1
